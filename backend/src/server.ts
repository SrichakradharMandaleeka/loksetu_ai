...
import { Issue, ProjectProposal, AIAnalysis, User, ChatMessage } from "./types";
...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    if (fs.existsSync(frontendDir)) {
      ...
      console.log("Vite development server mounted as middleware from:", frontendDir);
    } else {
      console.log("Frontend directory not found for Vite middleware, running standalone API server");
    }
  } else {
    ...
    if (fs.existsSync(distPath)) {
      ...
      console.log("Production static files server mounted from:", distPath);
    }
  }
